...
//прриклад звичайної функції
function myArray() {
  const a = [7, 8, 9];
  document.querySelector('.myFunc1').innerHTML = a.join('-');
}
myArray();

//а тепер робимо з неї колл бек

function myArray(callback,) {
  const a = [7, 8, 9];
  let element = document.querySelector('.myFunc1');
  callback(element, a)
}
...
function out2(element, arr) {
  element.innerHTML = arr.join('hi');
}

function out3(element, arr) {
  element.innerHTML = arr.map(elem => elem * 3);
}
function out4(element, arr) {
  element.innerHTML = arr.map(elem => elem/2)
}
